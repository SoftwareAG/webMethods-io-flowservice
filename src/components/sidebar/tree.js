...
const Tree = ({edges, location: {pathname}}) => {
  const parsedPathName = pathname.replace('/webmethods-io-flowservice', '');

  const [treeData] = useState(() => calculateTreeData(edges));

  let defaultCollapsed = {};

  treeData.items.forEach(item => defaultCollapsed[item.url] = !!(config.sidebar.collapsedNav
    && config.sidebar.collapsedNav.includes(item.url)));

  defaultCollapsed = Object.entries(defaultCollapsed)
  .map(([url, isCollapsed]) => parsedPathName.startsWith(url) ? [url, false] : [url, isCollapsed])
